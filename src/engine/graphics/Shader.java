...
public class Shader {
	...
	public void load(String vertPath, String fragPath) {
		/**
		 * This will load both the vert and frag files as a string in memory, then it
		 * will call the create method to compile the shaders
		 */
	...
	public void create(String vert, String frag) {
		/**
		 * This method will create a program and create vertex and fragment shaders Then
		 * it will compile the shaders and attach them to the program.
		 * 
		 * @param vert - vertex file in string format
		 * @param frag - fragment file in string format
		 */

		// Program which we will call with (GL20.glUseProgram(program)) when we want to
		// use the shaders
		...
		if (GL20.glGetShaderi(this.vertID, GL20.GL_COMPILE_STATUS) == GL20.GL_FALSE) {
			// This is the error checking for shaders, it will check if it compiled
			// correctly
	 ...
	 * Uniform setters
	 * 
	 * Basic format: glUniform1f(name, value); the function will set a uniform to
	...
	public void setUniform(String name, Matrix4f value) {
		...
		GL20.glUniformMatrix4fv(getUniformLocation(name), true, fb); // The true determines if it's in row major order,
																		// or col major order. It's true because it's in
																		// row major order.
	}
